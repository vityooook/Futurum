...
import {HighloadWallet, TIMEOUT_SIZE, TIMESTAMP_SIZE} from '../wrappers/HighloadWallet';
...
describe('HighloadWalletV3', () => {
    ...
    let highloadWalletV3: SandboxContract<HighloadWallet>;
    ...
    beforeEach(async () => {
        ...
        highloadWalletV3 = blockchain.openContract(
            HighloadWallet.createFromConfig(
                {
    ...
    it('should ignore internal transfer from address different from self', async () => {
        ...
        const transferBody = HighloadWallet.createInternalTransferBody({
            queryId,
            actions: [{
                type: 'sendMsg',
                mode: SendMode.PAY_GAS_SEPARATELY,
